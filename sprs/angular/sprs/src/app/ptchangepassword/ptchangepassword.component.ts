...
@Component({
  selector: 'app-ptchangepassword',
  templateUrl: './ptchangepassword.component.html',
  styleUrls: ['./ptchangepassword.component.css']
})
export class PtchangepasswordComponent implements OnInit {
  ...
  addDoctorPatient(){
    ...
    if (this.addpatientForm.dirty && this.addpatientForm.valid) {
      ...
      if(this.addpatientForm.value.new == this.addpatientForm.value.ret){
        ...
        this.loading = true;
        this.angularHttp.post("ptchnagepass.json", contact)
        .toPromise()
        .then((response) => {
