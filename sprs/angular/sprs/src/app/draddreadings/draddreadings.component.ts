...
@Component({
  selector: 'app-draddreadings',
  templateUrl: './draddreadings.component.html',
  styleUrls: ['./draddreadings.component.css'],
 // host: { '(document:click)': 'hostClick($event)' },
})
export class DraddreadingsComponent implements OnInit {
...
// }
  ...
  constructor(private formBuilder: FormBuilder, private router: Router, private angularHttp: AngularHttpProvider,private activatedroute: ActivatedRoute) {
    this.theError = "";
    this.FileSaveId = ''
    ...
    if (this.theError.length == 0) {
      this.angularHttp.post("addptReading.json", ptformData)
        ...
        .then((response) => {
          ...
          if (data.status == 200) {
            this.router.navigateByUrl('patient/readings/'+this.listID);
          } else {
